...
export default function SentimentAnalysis() {
  const handleGenerateReport = () => {
    if (sentimentOverview.length === 0) {
      toast.error('No sentiment data available');
      return;
    }
    downloadCSV('sentiment-report.csv', sentimentOverview.map(s => ({ label: s.label, value: s.value, count: s.count }))); 
