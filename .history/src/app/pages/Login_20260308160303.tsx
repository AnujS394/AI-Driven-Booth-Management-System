...
export default function Login() {
  ...
  const { login: contextLogin, isAuthenticated } = useUser();
  ...
  const handleForgotPassword = async () => {
    const input = window.prompt(
      'Enter your registered email or phone number for password reset:'
    );
    if (!input) {
      return;
    }
    try {
      await import('../services/auth').then((m) => m.sendPasswordReset(input));
